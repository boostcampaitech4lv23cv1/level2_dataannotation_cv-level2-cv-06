...
def image_size_dist(data_dict):
    ...
    g = sns.jointplot(data=image_df, x="image_width", y="image_height", kind="kde", space=0, color="r")
...
def image_tag_dist(data_dict):
    ...
    return plot_count_df(df=img_tag_df, field='image_tags', random_sample=False, color='g', rotation=0, xlabel='image tag', \
...
def word_tag_dist(data_dict):
    ...
    return plot_count_df(df=word_tag_df, field='word_tags', random_sample=False, color='g', rotation=0, xlabel='word tags', \
                ylabel='Count of each word tag', title='Word tag Distribution')


def word_per_image_dist(data_dict):
    image_df = data_dict['image_df']
    return plot_dist(df=image_df, field='word_counts', bins=50, color='b', xlabel='number of words per Image', \
...
def orientation_dist(data_dict):
    ...
    return plot_count_df(df=orientation, field='orientation', random_sample=False, color='g', rotation=0, xlabel='orientation', \
...
def language_dist(data_dict):
    ...
    return plot_count_df(df=lang, field='language', random_sample=False, color='g', rotation=0, xlabel='language', \
              ylabel='Count of each language', title='Language Distribution')


def bbox_size_dist(data_dict):
    word_df = data_dict['word_df']
    return plot_dist(df=word_df, field='bbox_size', bins=200, color='r', xlabel='BBOX size', \
          ylabel='Frequency', title='BBOX size')


def hor_aspect_ratio_dist(data_dict):
    hor_aspect_ratio = data_dict['hor_aspect_ratio']
    return plot_dist_list(hor_aspect_ratio, bins=20, color='r', xlabel='Aspect Ratio (BBOX Width / BBOX Height)', \
...
def ver_aspect_ratio_dist(data_dict):
    ...
    return plot_dist_list(ver_aspect_ratio, bins=20, color='r', xlabel='Aspect Ratio (BBOX Width / BBOX Height)', \
